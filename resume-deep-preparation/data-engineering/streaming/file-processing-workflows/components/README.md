...
**Your context:** You know PySpark; you did Glue but remember it vaguely; you haven’t worked on Redshift (you have an idea from the AWS Data Engineering exam). Use these docs to fill gaps and rehearse nuances.
...
2. **Then go component-by-component** in flow order when they drill down:
   - S3 → SNS/SQS → Orchestrator → DynamoDB → Glue (batch) → MSK → Spark Streaming → Redshift → Glue sync → RDBMS → API.
...
| 7 | `07-SPARK-STREAMING-REDSHIFT.md` | Spark Streaming + Redshift load | Micro-batch, staging table, overwrite by file_date, at-least-once |
| 8 | `08-REDSHIFT.md` | Redshift | Why Redshift; why not API direct; COPY, distribution/sort keys, connection limits |
| 9 | `09-GLUE-SYNC-RDBMS.md` | Glue sync (Redshift → RDBMS) | When it runs, full vs incremental, consistency, failure handling |
...
5. **Spark Streaming** (or Glue Streaming) consumes from **MSK**, aggregates for reporting, sinks to **Redshift** (staging → final; overwrite by file_date).
6. **Glue sync** job runs after pipeline load: reads **Redshift** (or S3), writes **RDBMS** (Aurora/PostgreSQL). **Spring Boot API** on EKS queries **RDBMS** (not Redshift); **Angular** app displays Daily Positions, Exposure by Borrower, Daily P&L.
